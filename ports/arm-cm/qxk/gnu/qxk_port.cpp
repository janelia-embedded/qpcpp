...
//
// NOTE: The IRQ priorities established in QXK_init() can be later changed
...
void QXK_init(void) {
    ...
    NVIC_IP[QXK_USE_IRQ_NUM >> 2U] &= ~(0xFFU << (8U*(QXK_USE_IRQ_NUM & 3U)));
...
//============================================================================
// The PendSV exception is used for handling asynchronous preemption in QXK.
...
#ifdef QXK_USE_IRQ_NUM  //--------- IRQ defined, use the specified IRQ
...
__asm volatile (
    ...
    "  BX      lr               \n" // exception-return to preempted *thread*
    );
}
...
__asm volatile (
    ...
    // NOTE: calling QF_int_enable_() corrupted the lr (EXC_RETURN), but it can
    // be synthesized because it is known (0xFFFFFFF9):
