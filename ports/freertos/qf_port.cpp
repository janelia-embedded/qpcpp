                         ...
                         std::uint_fast8_t const qsId) noexcept
{
    ...
    if (m_nFree > static_cast<QMPoolCtr>(margin)) {
        ...
        Q_ASSERT_INCRIT(800, fb != nullptr);
        ...
        Q_ASSERT_INCRIT(802, Q_PTR2UINT_CAST_(fb_next)
        ...
        if (m_nFree == 0U) {
            ...
            Q_ASSERT_INCRIT(830, QF_PTR_RANGE_(fb_next, m_start, m_end));
        ...
        QS_BEGIN_PRE(QS_QF_MPOOL_GET, qsId)
            ...
            QS_MPC_PRE(m_nFree);   // # free blocks in the pool
                        ...
                        std::uint_fast8_t const qsId) noexcept
{
...
#ifndef Q_UNSAFE
    fb->m_next_dis = static_cast<uintptr_t>(~Q_PTR2UINT_CAST_(fb->m_next));
#endif
    ...
    QS_BEGIN_PRE(QS_QF_MPOOL_PUT, qsId)
        QS_TIME_PRE();         // timestamp
        QS_OBJ_PRE(this);      // this memory pool
        QS_MPC_PRE(m_nFree);   // the number of free blocks in the pool
    QS_END_PRE()
